...
QtCGI* QtCGI::ProcessPost()
{
	...
	if (this->mRequestHeaders.value(QtCGI::HeaderRequestMethod).toUpper() == QtCGI::MethodPost) {
		...
		if (!QString::fromStdString(strPostData).isEmpty()) {
            ...
            this->mPostStrParameters = QString::fromStdString(strPostData);
		}
	}
	// Return the instance
	return this;
...
QMap<QString, QString> QtCGI::DecodeQuery(QString strQuery, QString strPairSeparator)
{
	...
	foreach (QString strPair, qslParameters) {
        ...
        qmQuery.insert(QUrl::fromPercentEncoding(qslPair.at(0).toLatin1()).replace("+", " "),
                       QUrl::fromPercentEncoding(qslPair.at(1).toLatin1()).replace("+", " "));
	}
	// Return the map
	return qmQuery;
}
 ...
 * @return QString QtCGI::mPostStrParameters
 */
QString QtCGI::GetPostData()
{
    // Return the POST data
    return this->mPostStrParameters;
}

QString QtCGI::GetRequestMethod()
{
    return mRequestHeaders.value(QtCGI::HeaderRequestMethod).toUpper();
}
